...
function parseInvoiceFolio(raw: string): { type: "FACTURA" | "BOLETA"; folio: string } | null {
  ...
  if (cotMatch) return { type: "FACTURA", folio: `COT-${cotMatch[1].trim()}` };

  // Guía/Guia de despacho N XXXXX
  const guiaMatch = trimmed.match(/^Gu[ií]a\s+de\s+despacho\s+N\s+(.+)$/i);
  if (guiaMatch) return { type: "FACTURA", folio: `GD-${guiaMatch[1].trim()}` };
