...
export type CreateGuestState = { error?: string; guest?: { id: string; fullName: string; email: string; type: "PERSON" | "COMPANY" | "CLUB" | "DELEGACION" } };
...
export async function createGuest(
  ...
  const guestTypeRaw = formData.get("guestType")?.toString()?.toUpperCase();
  const guestType =
    guestTypeRaw === "COMPANY"
      ? "COMPANY"
      : guestTypeRaw === "CLUB"
        ? "CLUB"
        : guestTypeRaw === "DELEGACION"
          ? "DELEGACION"
          : "PERSON";
  ...
  if ((guestType === "COMPANY" || guestType === "DELEGACION") && !companyName) {
    return { error: guestType === "DELEGACION" ? "El nombre de la delegación es obligatorio" : "La razón social es obligatoria para empresas" };
  }
  ...
  try {
    const guest = await prisma.guest.create({
      data: {
        ...
        type: guestType as "PERSON" | "COMPANY" | "CLUB" | "DELEGACION",
        fullName,
        email: email ?? null,
        phone: phone!,
        rut: rut ?? null,
        emergencyContact,
        nationality,
        ...(guestType === "COMPANY" || guestType === "DELEGACION" ? { companyName, companyRut, companyEmail } : {}),
      },
