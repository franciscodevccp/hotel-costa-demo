                ...
                filtered.map((p) => (
                  <tr key={p.id} className="border-b border-[var(--border)] hover:bg-[var(--background)]/50">
                    ...
                    <td className="px-4 py-3 text-[var(--muted)]">
                      ...
                      {p.paymentType === "RECURRING" && p.intervalDays != null && p.intervalDays > 0 && (
                        <><br /><span className="text-xs text-[var(--primary)]">Cada {p.intervalDays} día{p.intervalDays !== 1 ? "s" : ""}</span></>
                      )}
                      {p.paymentType !== "RECURRING" && p.entryDate && p.dueDate && (() => {
            ...
            <form action={createAction} className="space-y-4">
              ...
              <div>
                ...
                <p className="mt-1 text-xs text-[var(--muted)]">
                  {createPaymentType === "SINGLE" ? "Sin mensualidad; solo proveedor y monto." : "Se configura fecha de pago inicial y cada cuántos días se paga."}
                </p>
...
function EditModal({
  ...
  const [editPaymentType, setEditPaymentType] = useState<"SINGLE" | "RECURRING">(item.paymentType === "RECURRING" ? "RECURRING" : "SINGLE");
          ...
          <>
            ...
            <div>
              ...
              <input
                ...
                className="w-full rounded-lg border border-[var(--border)] bg-[var(--background)] px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[var(--primary)]"
              />
              <input type="hidden" name="intervalDays" value={editIntervalDays} />
            </div>
          </>
        )}
        <div>
