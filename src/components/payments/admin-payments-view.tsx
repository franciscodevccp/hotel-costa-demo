...
/** Fila de tabla: pago registrado o reserva sin abono (solo saldo pendiente). */
export type PaymentTableRow = PaymentRow & { isPendingOnly?: boolean };
...
export function AdminPaymentsView({
  ...
  useEffect(() => {
    ...
    if (pend) {
      ...
      // No hacer router.replace aquí: mantener ?reservation=id para que el card "Registrar primer pago" siga visible
    }
  ...
  const pendingOnlyRows: PaymentTableRow[] = pendingReservations
    ...
    .map((r) => ({
      ...
      registered_by: "—",
      reservation_status: "confirmed" as ReservationStatusRow,
      isPendingOnly: true,
    }));
  ...
  if (search) {
    ...
    filtered = filtered.filter(
      (p) =>
        ...
        (p.registered_by && p.registered_by.toLowerCase().includes(q))
    );
  }
